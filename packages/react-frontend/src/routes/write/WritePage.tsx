...
export default function WritePage() {
  ...
  const [status, setStatus] = useState<
    { id: number; msg?: string } | undefined
  >(undefined);
  const [errorMsg, setErrorMsg] = useState<string | undefined>(undefined);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [userDiaries, setUserDiaries] =
    useState<{ id: string; title: string }[]>(undefined);
  ...
  useEffect(() => {
    ...
    const loadDiaries = async () => {
      ...
      if (diaries && diaries.length > 0) {
        ...
        if (selectedIndex >= 0) setSelectedDiary(selectedIndex);
      }
    };
    loadDiaries()
          ...
          <div className="flex flex-row gap-6 items-center">
            ...
            >
              ...
              <div className="flex flex-row items-center p-2 border-2 border-secondary-300 rounded-lg">
                {typeof selectedDiary === "number"
                  ? userDiaries[selectedDiary].title
                  : "Choose diary"}
                {dropdownOpen ? (
              ...
              {/* Opened */}
              <ul
                className={`${!dropdownOpen && "hidden"} absolute top-0 left-0 bg-primary-600 flex flex-col items-center justify-center border-2 border-secondary-300 rounded-lg overflow-hidden`}
              >
                {userDiaries?.map((diary, i) => (
                  <Fragment key={i}>
                    <li
                      className={`${selectedDiary === i && "bg-primary-800"} hover:bg-primary-800 p-2 w-full text-nowrap`}
