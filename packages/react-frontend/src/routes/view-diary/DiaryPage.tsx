...
function DiaryEntry({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row p-4 justify-between items-center">
        ...
        >
          ...
          <button
            ...
            onClick={() =>
              navigate(`/app/write?diary=${diary._id}&page=${page._id}`)
            }
          >
      ...
      <div className="flex flex-col bg-primary-600 min-h-full overflow-y-scroll px-4 pb-24 gap-3">
        ...
        <div className="w-full flex flex-col">
          ...
          <span className="bg-primary-500 p-3 gap-3 flex flex-col rounded-br-lg rounded-bl-lg">
            ...
            {page.likeCounter && (
              <span>
                <b>Likes 😻</b> {page.likeCounter}{" "}
              </span>
            )}
            {page.comments && (
              <>
                <span>
                  <b>Comments</b> {page.comments.length}
                </span>
...
function DiaryPage() {
  ...
  const fetchDiary = async () => {
    try {
      ...
      const diary = (await getUserDiaries()).find((d) => d._id === diaryId);

      if (!diary) setError("Diary not found.");
      act(() => {
        setDiary(diary);
      });
    } catch (error) {
