...
export async function authenticatedRoute(req, res, next) {
    ...
    console.log(
        "Attempted authentication validation failed with message:",
    ...
    if (userValidation.errorMessage === "ACCESS_TOKEN_EXPIRED") {
        try {
            ...
            res.setHeader(
        "Set-Cookie",
        cookie.serialize("auth", JSON.stringify(newCredentials)),
            );
            ...
            res
                .status(500)
                .send(`Authentication Unexpectedly Failed With Error: ${e}`);
        }
    } else {
        // Unrecoverable cases send same response
        res
            .status(401)
            .send(`Authentication Failed: ${userValidation.errorMessage}`);
    }
}
